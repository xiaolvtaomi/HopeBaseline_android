...
import com.baseline.android.frame.utils.DateUtil;
...
//    }
    ...
    public View getView(Context context, Cursor cursor)
    {
        ...
        // 1.1 获取接收类型消息展示view
        if (typeSendOrReceive == BaseMessageModel.MSGSENDORRECV_RECV)
        {
            //接收，初始化接收View           
        ...
        msgContainer.setTag(R.string.im_msg_menu_transfer, msgCanBeTransfer());
    ...
    {   
        ...
        // 接收到的消息状态展示
        if (getTypeSendOrReceive() == BaseMessageModel.MSGSENDORRECV_RECV)
        {
            if (readFlagIV != null)
            {
        ...
        // 发送出去的消息状态展示
        else if (getTypeSendOrReceive() == BaseMessageModel.MSGSENDORRECV_SEND)
        {
            msgStatusTV.setText("");
            ...
            {       
                //2.上传进度的显示
                //如果是音频显示进度条
                if (msg.getMsgType() == MediaIndexModel.MEDIATYPE_AUDIO)
                {
                    //进度文本显示隐藏
                ...
                //地图的气泡左边不显示进度
                else if (msg.getMsgType() == MediaIndexModel.MEDIATYPE_LOCATION)
                {
                    //显示消息状态
            ...
            if (resendBtn != null && msgStatusTV != null)
            {
                ...
                else
                {
                    ...
                    //非发送失败的其他状态
                    //1、如果是音频
                    if (msg.getMsgAttachment() != null
                            && msg.getMsgType() == MediaIndexModel.MEDIATYPE_AUDIO)
                    {
                        //正在发送和阻塞状态显示进度条
